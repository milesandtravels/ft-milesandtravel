<template>
  <div class="campaigns-preview-section mb-8">
    ...
    <div class="d-flex align-center justify-space-between mb-6">
      <div>
        <h2 class="text-h4 font-weight-bold mb-2">
          <v-icon icon="mdi-bullhorn" color="primary" size="32" class="mr-2"></v-icon>
          Campanhas Ativas
        </h2>
        ...
        <div class="d-flex align-center">
          <div class="featured-content pa-6 flex-grow-1">
            ...
            <h3 class="text-h5 font-weight-bold mb-2">
              Ganhe até {{ formatValue(featuredCampaign.value, featuredCampaign.value_type) }}
            </h3>
          ...
          <div class="featured-logos pa-4">
            <div v-if="featuredCampaign.type === 'bonus_transfer'" class="transfer-logos">
              <div class="logo-item text-center">
                ...
                <p class="text-caption mt-1">{{ featuredCampaign.program_origin.name }}</p>
              </div>
              
              <v-icon icon="mdi-arrow-right-bold" size="20" color="primary" class="mx-2"></v-icon>
              ...
              <div class="logo-item text-center">
                <div class="logo-container">
                  ...
                  >
                    <template v-slot:error>
                      <v-icon icon="mdi-airplane" size="20" color="grey"></v-icon>
                    </template>
                  </v-img>
                </div>
                <p class="text-caption mt-1">{{ featuredCampaign.program_destination.name }}</p>
              </div>
            </div>
            ...
            <div v-else class="single-logo text-center">
              ...
              <p class="text-body-2 font-weight-medium mt-2">{{ featuredCampaign.program_destination.name }}</p>
            </div>
          </div>
        </div>
      </v-card>
      ...
      <v-row>
        <v-col cols="12" sm="6">
          ...
          >
            <div class="d-flex align-center justify-space-between">
              <div>
                <h4 class="text-h6 font-weight-bold mb-1">{{ campaigns.length }} Campanhas</h4>
                <p class="text-body-2 text-medium-emphasis mb-0">Ver todas as ofertas ativas</p>
              </div>
        ...
        <v-col cols="12" sm="6">
          ...
          >
            <div class="d-flex align-center justify-space-between">
              <div>
                <h4 class="text-h6 font-weight-bold mb-1">{{ activeCampaignsCount }} Válidas</h4>
                <p class="text-body-2 text-medium-emphasis mb-0">Campanhas ainda ativas</p>
              </div>
    ...
    <v-card v-else class="pa-8 text-center" variant="outlined">
      <v-icon icon="mdi-bullhorn-outline" color="grey" size="64" class="mb-4"></v-icon>
...
const featuredCampaign = computed(() => {
  ...
  const activeCampaigns = campaigns.value.filter(c => !isCampaignExpired(c.expiration_date))
...
const formatValue = (value: number, valueType: string): string => {
  return valueType === 'percent' ? `${value}%` : `${value.toLocaleString('pt-BR')} pontos`
}
