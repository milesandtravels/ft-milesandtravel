<template>
  <v-container class="pa-2 pa-md-4">
    ...
    <!-- Results Header -->
    <div v-if="filteredEcommerces.length > 0" class="results-header mb-3 mb-md-4 px-2 px-md-0">
      <h3 class="text-h6 font-weight-bold">
        {{ filteredEcommerces.length }} loja{{ filteredEcommerces.length > 1 ? 's' : '' }} encontrada{{ filteredEcommerces.length > 1 ? 's' : '' }}
      </h3>
    </div>
    ...
    <!-- Empty State -->
    <div v-else-if="filteredEcommerces.length === 0 && !loading && hasFiltered" class="empty-state text-center py-8 py-md-12">
      <v-icon class="mb-3" color="grey-lighten-1" size="48">mdi-store-search-outline</v-icon>
      <h3 class="text-body-1 text-md-h6 font-weight-medium mb-2">Nenhuma loja encontrada</h3>
      <p class="text-body-2 text-medium-emphasis">
    ...
    <div v-else-if="!hasFiltered" class="initial-state text-center py-8 py-md-12">
      ...
      <h3 class="text-body-1 text-md-h6 font-weight-medium mb-2">Explore nossos e-commerces!</h3>
      <p class="text-body-2 text-medium-emphasis">
    ...
    <!-- End of Results -->
    <div v-if="!hasMore && !loading && ecommerces.length > 0" class="text-center py-6">
      <v-alert
        class="mx-auto"
        density="compact"
              ...
              <div class="d-flex align-center mb-4">
                ...
                >
                  <v-icon
                    ...
                    size="24"
                  >
                    {{ category === 'Pontos e Milhas' ? 'mdi-airplane' : 'mdi-cash-multiple' }}
                  </v-icon>
                </v-avatar>
                <div>
                  <h2 class="text-h6 font-weight-bold">{{ category }}</h2>
                  <p class="text-caption text-medium-emphasis">
                    {{ platforms.length }} {{ platforms.length === 1 ? 'plataforma disponível' : 'plataformas disponíveis' }}
                  </p>
                </div>
              </div>
...
<script setup lang="ts">
  ...
  const benefitPlatforms = ref<Record<string, BenefitPlatform[]>>({
    'Pontos e Milhas': [
      { name: 'Esfera', cashback: '5% de CashBack', logo: '🌟', color: 'green' },
    ...
    'CashBack': [
      { name: 'Méliuz', cashback: '12% de CashBack', logo: '💰', color: 'pink' },
      { name: 'Mobils', cashback: '8% de CashBack', logo: '📱', color: 'purple' },
      { name: 'PicPay', cashback: '15% de CashBack', logo: '💳', color: 'green' },
      { name: 'Luiza BB', cashback: '8% de CashBack', logo: '⭐', color: 'amber' },
      { name: 'BusCoop', cashback: '6% de CashBack', logo: '🚌', color: 'blue' },
      { name: 'Zoom', cashback: '6% de CashBack', logo: '🔍', color: 'grey' },
      { name: 'Banco Pan', cashback: '6% de CashBack', logo: '🏦', color: 'indigo' },
      { name: 'Banco Inter', cashback: '5% de CashBack', logo: '💼', color: 'deep-orange' },
    ],
  })
  ...
  const searchEcommerces = (query: string) => {
    ...
    setTimeout(() => {
      const filtered = ecommerces.value.filter(ecommerce =>
  ...
  const handleFilter = async () => {
    ...
    setTimeout(() => {
      ...
      if (searchQuery.value || selectedEcommerce.value) {
        ...
        filtered = filtered.filter(ecommerce =>
          ecommerce.name.toLowerCase().includes(query.toLowerCase())
          || ecommerce.category.toLowerCase().includes(query.toLowerCase()),
        )
      }
  ...
  console.log('💡 Teste com: "Casas Bahia", "Amazon", "Magazine", "Marketplace"')
</script>
