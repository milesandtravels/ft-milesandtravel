      ...
      <v-row>
        <v-col cols="12">
          ...
          <v-row v-else>
            ...
            >
              <v-card
                class="program-card elevation-2 h-100"
                :ripple="false"
              >
...
useHead({
  ...
  meta: [
    {
      name: 'description',
      content: 'Configure os preços dos programas de pontos e milhas para otimizar suas buscas'
    }
  ]
})
...
const savePrice = async () => {
  ...
  try {
    const { error: apiError } = await useSanctumFetch(
      `/api/program-users`,
      {
        method: 'POST',
        body: {
    ...
    const programIndex = programs.value.findIndex(p => p.id === selectedProgram.value!.id)
